...
const uploadFirebase = async (courseData, yearsData) => {
  try {
    ...
    const courseQuerySnapshot = await getDocs(collection(firestore, "courses"));
    const existingCourses = courseQuerySnapshot.docs.map(doc => doc.data().name.toLowerCase());
    ...
    const existingYears = yearsQuerySnapshot.docs.map(doc => doc.data().year);
    const newYears = yearsData.filter(year => !existingYears.includes(year));
    ...
    const courseYearQuerySnapshot = await getDocs(collection(firestore, "courseyear"));
    const existingCourseYears = courseYearQuerySnapshot.docs.map(doc => {
      const data = doc.data();
      return `${data.course.toLowerCase()}_${data.year}`;
    });
...
const handleSubmit = async (e) => {
  ...
  // Clear errors if fields are valid
  setErrors({});
  await uploadFirebase(course, selectedYears.map((year) => year.label));
};
  ...
  return (
   ...
   <form onSubmit={handleSubmit} className="bg-white shadow-md rounded px-8 pt-6 pb-8">
          ...
          <div className="mb-4 w-[500px]">
            ...
            <CreatableSelect
            options={yearsCollection.map(year => ({ value: year.id, label: year.year }))}
            value={selectedYears}
